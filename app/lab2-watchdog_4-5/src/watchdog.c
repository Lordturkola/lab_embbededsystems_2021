...
//4.5
#define EXTRA_INPUT_FLAG    (0x3F<<4) // 4 to 9 , surmising buttons actually mapped to those positions
...
#define CONTROL_PERIOD  300 // 300
#define VEHICLE_PERIOD  300 // 300
/////for 4.5
...
PERF_END(PERFORMANCE_COUNTER_BASE,1);
    ...
    perf_print_formatted_report((void*)PERFORMANCE_COUNTER_BASE,
      alt_get_cpu_freq(), 1, "Watchdog_Timer");
...
void ExtraTask(void* pdata)
{
  ...
  while(1)
  {
    ...
    //msg = *((int*)OSMboxPend(Mbox_Util,1,&err));// data shifted to lowest bits in switchtask
    int switch_values = switches_pressed();
...
*/
    ...
    Oneshot_Delay_Timer = OSTmrCreate(delay,0
      ,OS_TMR_OPT_ONE_SHOT,(void*)0,(void*)0,"One-shot Timer",&err2);
    ...
    //OSTmrStart(Oneshot_Delay_Timer,&err); // start one-shot
   OSTmrDel(Oneshot_Delay_Timer,&err2);
  }
}
///////////
/* 
...
*/
  ...
  // 4.5
  int test = 1;
  Mbox_Util = OSMboxCreate((void*) &test); // to send switch/ overload data  
