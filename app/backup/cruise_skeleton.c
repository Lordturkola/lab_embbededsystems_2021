...
*/
OS_TMR_CALLBACK callback_tmr_vehicle(void *ptmr, void *parg){
  //signal to vhicle task that it's released
 INT8U err = OSSemPost(Vehicle_sem);
}

OS_TMR_CALLBACK callback_tmr_control(void *ptmr, void *parg){
  INT8U err = OSSemPost(Control_sem);
}

OS_TMR_CALLBACK callback_tmr_SwitchIO(void *ptmr, void *parg){
  printf("callback_tmr_SwitchIO\n");
 INT8U err = OSSemPost(SwitchIO_sem);
}

OS_TMR_CALLBACK callback_tmr_ButtonIO(void *ptmr, void *parg){
  printf("callback_tmr_ButtonIO\n");
  INT8U err = OSSemPost(ButtonIO_sem);
}
/*
 * ISR for HW Timer
 */
...
{ 
  ...
  while(1){
    OSSemPend(SwitchIO_sem,0,perr);

    swVal = switches_pressed();
    printf("button vector %d \n", swVal);
    if(swVal & ENGINE_FLAG)
        *msg = on; 
    else
         *msg = off; 
        
    OSMboxPost(Mbox_Engine,(void*)msg);

    if(swVal & TOP_GEAR_FLAG)
        *msg = on; 
    else
      *msg = off; 

    OSMboxPost(Mbox_Gear,(void*)msg);
  }
}
...
void ControlTask(void* pdata)
{
  ...
  msg = (void*)((enum active) 2);
  while(1)
  {
    OSSemPend(Control_sem,0,&err); // wait for period  then execute
...
void StartTask(void* pdata)
{
  ...
  */
   ...
   */
  Vehicle_tmr = OSTmrCreate(0,6,OS_TMR_OPT_PERIODIC,callback_tmr_vehicle,(void*)0,"Vehicle timer",&err);
  Control_tmr = OSTmrCreate(0,6,OS_TMR_OPT_PERIODIC,callback_tmr_control,(void*)0,"Control timer",&err);
  SwitchIO_tmr = OSTmrCreate(0,2,OS_TMR_OPT_PERIODIC,callback_tmr_SwitchIO,(void*)0,"SwitchIO timer",&err);
  ButtonIO_tmr = OSTmrCreate(0,2,OS_TMR_OPT_PERIODIC,callback_tmr_ButtonIO,(void*)0,"ButtonIO timer",&err);
  OSTmrStart(Vehicle_tmr,&err);
  OSTmrStart(Control_tmr,&err);
  OSTmrStart(ButtonIO_tmr,&err);
  OSTmrStart(SwitchIO_tmr,&err);
  ...
  Mbox_Gear = OSMboxCreate((void*) 1); /* Empty Mailbox - Throttle */
  Mbox_GasPedal = OSMboxCreate((void*) 1); /* Empty Mailbox - Velocity */
  Mbox_CruiseControl = OSMboxCreate((void*) 1); /* Empty Mailbox - Velocity */
  /*
