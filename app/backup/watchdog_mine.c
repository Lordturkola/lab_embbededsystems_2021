...
void ControlTask(void* pdata)
{
  ...
  while(1)
  {
    ...
    if(cruise_control == on)
      throttle += k*((float)current_velocity - target_velocity);
    // current_velocity = (INT16S*) msg;
...
void WatchdogTask(void* pdata)
{
  ...
  INT8U signal = 0;   // enum didn not run = 0 , did run = 1
  INT8U err; 
  while(1)
  {// sem initialized as 0; run only at end of hyperperiod
    OSSemPend(Watchdog_Sem,0,&err); 
    signal = *((int*)OSMboxPend(Mbox_Watchdog_OK_Signal,0,&err)); // get message 
