...
// Semaphores
OS_EVENT *VechicleTaskTimerSem; 
OS_EVENT *ControlTaskTimerSem; 
OS_EVENT *ButtonIOTaskTimerSem; 
...
OS_TMR_CALLBACK vechicleTaskCallback(void *ptmr, void *parg) {
  INT8U err; 
  // printf("Vechicle task callback\n");
  err = OSSemPost(VechicleTaskTimerSem); 
} 

OS_TMR_CALLBACK ControlTaskCallback(void *ptmr, void *parg){
  INT8U err; 
  // printf("Control task callback\n");
  err = OSSemPost(ControlTaskTimerSem); 
} 

OS_TMR_CALLBACK ButtonIOTaskCallback(void *ptmr, void *parg){
  INT8U err; 
  // printf("Button task callback\n");
  err = OSSemPost(ButtonIOTaskTimerSem); 
} 

OS_TMR_CALLBACK SwitchTaskCallback(void *ptmr, void *parg){
  INT8U err; 
  // printf("Switch task callback\n");
  err = OSSemPost(SwitchTaskTimerSem); 
} 
...
{ 
  ...
  while(1)
  {
    ...
    OSSemPend(VechicleTaskTimerSem, 0, &err);  
...
void ButtonIOTask(void* pdata)
{
  ...
  while(1)
  {
    ...
    // Use soft timer to implement periodic task
    OSSemPend(ButtonIOTaskTimerSem, 0, &err);  
...
void SwitchTask(void* pdata)
{
  ...
  while(1)
  {
    ...
    if ((switch_values & ENGINE_FLAG)) engine = on;
    else engine = off;

    // Check if gear is high
    if (switch_values & TOP_GEAR_FLAG) top_gear = on;
    else top_gear = off;

    // send messages to mailboxes
    err = OSMboxPost(Mbox_Engine, (void *)&top_gear);
    err = OSMboxPost(Mbox_Gear, (void *)&engine);
...
void ControlTask(void* pdata)
{
  ...
  while(1)
  {
    ...
    OSSemPend(ControlTaskTimerSem, 0, &err); 
  }
}
...
void StartTask(void* pdata)
{
  ...
  VechicleTaskTimerSem = OSSemCreate(1);
  ControlTaskTimerSem = OSSemCreate(1);
  ButtonIOTaskTimerSem = OSSemCreate(1);
  SwitchTaskTimerSem = OSSemCreate(1);



    VehicleTaskTimer = OSTmrCreate(0,
      6,
      OS_TMR_OPT_PERIODIC,
      vechicleTaskCallback,
      (void *)0,
      "Vehicle Task Timer",
      &err);

    ControlTaskTimer = OSTmrCreate(0,
      6,
      OS_TMR_OPT_PERIODIC,
      ControlTaskCallback,
      (void *)0,
    ...
    ButtonTaskTimer = OSTmrCreate(0,
      2,
      OS_TMR_OPT_PERIODIC,
      ButtonIOTaskCallback,
      (void *)0,
      "ButtonIO Task Timer",
      &err);

    SwitchTaskTimer = OSTmrCreate(0,
      2,
      OS_TMR_OPT_PERIODIC,
      SwitchTaskCallback,
      (void *)0,
